...
fn interface(comptime T: type) Interface {
    ...
    comptime {
        ...
        for (@typeInfo(Impl).Struct.fields) |field| {
            const fnInfo = @typeInfo(field.field_type).Fn;
            const helperInfo = @typeInfo(@TypeOf(@field(T, field.name))).Fn;

            compileAssert(fnInfo.calling_convention == helperInfo.calling_convention,
                "method " ++ @typeName(T) ++ ".Impl." ++ field.name ++ " does not match helper function " ++ @typeName(T) ++ "." ++ field.name ++ ": different calling convention");
            compileAssert(fnInfo.alignment == helperInfo.alignment,
                "method " ++ @typeName(T) ++ ".Impl." ++ field.name ++ " does not match helper function " ++ @typeName(T) ++ "." ++ field.name ++ ": different alignment");
            compileAssert(fnInfo.is_generic == helperInfo.is_generic,
                "method " ++ @typeName(T) ++ ".Impl." ++ field.name ++ " does not match helper function " ++ @typeName(T) ++ "." ++ field.name ++ ": different arguments");
            compileAssert(fnInfo.is_var_args == helperInfo.is_var_args,
                "method " ++ @typeName(T) ++ ".Impl." ++ field.name ++ " does not match helper function " ++ @typeName(T) ++ "." ++ field.name ++ ": different arguments");
            compileAssert(fnInfo.return_type.? == helperInfo.return_type.?,
                "method " ++ @typeName(T) ++ ".Impl." ++ field.name ++ " does not match helper function " ++ @typeName(T) ++ "." ++ field.name ++ ": different return type");
            compileAssert(fnInfo.args.len == helperInfo.args.len,
                "method " ++ @typeName(T) ++ ".Impl." ++ field.name ++ " does not match helper function " ++ @typeName(T) ++ "." ++ field.name ++ ": different number of arguments");

            var i: usize = 0;
            while (i < fnInfo.args.len) : (i+=1) {
                compileAssert(fnInfo.args[i].is_generic == helperInfo.args[i].is_generic,
                    "argument " ++ itoa(i) ++ " of method " ++ @typeName(T) ++ ".Impl." ++ field.name ++
                    " does not match corresponding argument in helper function "++ @typeName(T) ++ "." ++ field.name);
                compileAssert(fnInfo.args[i].is_noalias == helperInfo.args[i].is_noalias,
                    "argument " ++ itoa(i) ++ " of method " ++ @typeName(T) ++ ".Impl." ++ field.name ++
                    " does not match corresponding argument in helper function "++ @typeName(T) ++ "." ++ field.name);
                if (fnInfo.args[i].arg_type.? == *This or fnInfo.args[i].arg_type.? == *const This) {
                    compileAssert(helperInfo.args[i].arg_type.? == T,
                        "argument " ++ itoa(i) ++ " of method " ++ @typeName(T) ++ ".Impl." ++ field.name ++
                        " does not match corresponding argument in helper function "++ @typeName(T) ++ "." ++ field.name);
                } else {
                    compileAssert(fnInfo.args[i].arg_type.? == helperInfo.args[i].arg_type.?,
                        "argument " ++ itoa(i) ++ " of method " ++ @typeName(T) ++ ".Impl." ++ field.name ++
                        " does not match corresponding argument in helper function "++ @typeName(T) ++ "." ++ field.name);
                }
            }
    ...
    const Storage = struct {
        const impl_ = blk: {
            comptime {
                ...
                for (@typeInfo(Impl).Struct.fields) |field| {
                    const expectedTypeInfo = @typeInfo(field.field_type).Fn;
